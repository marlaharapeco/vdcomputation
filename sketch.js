...
function setup() {
  ...
  archivePanel = createDiv();
  archivePanel.addClass('archive-panel');

  loadArchive();

  clearArchiveBtn = createButton('clear the archive');
  clearArchiveBtn.addClass('clear-archive-btn');
  ...
  refreshBtn.position(
    canvas.position().x + width + 65,
    canvas.position().y + height + 50
  );
  ...
  btn.position(
    canvas.position().x + width + 80,
    canvas.position().y + height + 80
  );
...
function addToArchive(imgData) {
  let images = JSON.parse(localStorage.getItem('archiveImages')) || [];
  images.push(imgData);
  localStorage.setItem('archiveImages', JSON.stringify(images));

  addThumbnail(imgData);
}

function addThumbnail(imgData) {
  let thumb = createImg(imgData);
  thumb.addClass('archive-thumb');
...
function clearArchive() {
  localStorage.removeItem('archiveImages');
  archivePanel.html('');
}